...
async function showModalInfo(modalType) {
    ...
    switch (modalType) {
        ...
        case "create-playlist":
            ...
            publicContainer.classList.add("publicContainer")
            publicContainer.append(publicLabel)
            publicContainer.append(publicField)
            ...
            modalContent.appendChild(publicContainer);
        ...
        case "search":
            ...
            searchTitle.innerText = "Búsqueda"
            ...
            middleContainer.appendChild(searchInput);
            searchBar.appendChild(middleContainer);
            searchInput.addEventListener("keydown", function (e) {

// fixme faltaría ponerle que si la cadena está vacía, que no cargue nada
                setTimeout(() => {
                    search(searchInput.value)
                }, 700)

            })
            modalContent.appendChild(searchTitle)
...
async function search(nombre) {
    ...
    try {
        const response = await fetch(`http://${APP_IP}/api/songs/song/name?nombre=${nombre}`);
        if (!response.ok) {
            throw new Error(`Error en la solicitud: ${response.status}`);
        }
        const data = await response.json();
...
function loadTrackList(data) {
    ...
    contenedor.classList.add("trackContainer");
    contenedor.innerHTML = ""
    for (let d of data) {
        let cancionItem = document.createElement("div")
        cancionItem.classList.add("songSearchContainer")

        let songSearchImage = document.createElement("img")
        songSearchImage.height = 20
        songSearchImage.src = d.image
        cancionItem.onclick = () => {
            play(d)
        }

        let songSearchText = document.createElement("p")
        songSearchText.innerHTML = d.name + " · <span>" + d.artists[0].name + "</span>"

        cancionItem.appendChild(songSearchImage)
        cancionItem.appendChild(songSearchText)
        contenedor.appendChild(cancionItem)
